...
import { mq, spacing, screenReadersOnlyMixin } from "../theme"
...
LayoutProps) => {
  return (
    <>
      <Skip href="#page-main">{`general.skip_to_content`}</Skip>
      <Page
      ...
      >
        <div>
          ...
          >
            <ScreenReadersHint>{`general.open_nav`}</ScreenReadersHint>
            <Hamburger />
          </MenuToggle>
          <Sidebar
            {...props}
...
const Page = styled.div.attrs(({ showSidebar }: { showSidebar: boolean }) => {
  showSidebar
})`
  @media ${mq.large} {
    display: grid;
