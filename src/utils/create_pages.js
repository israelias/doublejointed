...
const rawSitemap = yaml.load(
  fs.readFileSync(
    path.resolve(
      __dirname,
      `../surveys/${process.env.SURVEY}/config/raw_sitemap.yml`
    ),
    "utf8"
  )
)
const config = yaml.load(
  fs.readFileSync(
    path.resolve(
      __dirname,
      `../surveys/${process.env.SURVEY}/config/config.yml`
    ),
    "utf8"
  )
)
...
exports.createPagesSingleLoop = async ({
  graphql,
  ...
  for (const page of flat) {
    ...
    for (let index = 0; index < locales.length; index++) {
      ...
      const pageObject = {
        ...
        component: path.resolve(`./src/components/pages/pageTemplate.tsx`),
        context: {
          ...context,
...
exports.createPagesTwoLoops = async ({
  graphql,
  actions: { createPage, createRedirect },
}) => {
  let allQueries = ""
