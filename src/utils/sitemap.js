...
const rawPageTemplates = fs.readFileSync(
  `.src/surveys/${process.env.SURVEY}/config/page_templates.yml`,
  "utf8"
)
const rawBlockTemplates = fs.readFileSync(
  `.src/surveys/${process.env.SURVEY}/config/block_templates.yml`,
  "utf8"
)
const globalVariables = yaml.load(
  fs.readFileSync(
    `.src/surveys/${process.env.SURVEY}/config/variables.yml`,
    "utf8"
  )
)
...
const applyTemplate = (config, templateName, rawTemplates, parent) => {
  ...
  const populatedTemplate = injectVariables(
    templateObject,
    variables,
    templateName
  )
  ...
  try {
    ...
    if (Array.isArray(config.children)) {
      config.children.forEach(child => {
        page.children.push(
          exports.pageFromConfig(stack, child, page, pageIndex)
        )
      })
    }
...
exports.computeSitemap = async (rawSitemap, locales) => {
  try {
    ...
    await fs.writeFileSync(
      `.src/surveys/${process.env.SURVEY}/config/sitemap.yml`,
      sitemapContent
    )
    await fs.writeFileSync(
      `.src/surveys/${process.env.SURVEY}/config/blocks.yml`,
